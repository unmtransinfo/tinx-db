...
# MySQL params - you will need to tune these based on your available resources (defaults are for 32GB RAM, 8 cores)
INNODB_BUFFER_POOL_SIZE=20G # InnoDB buffer pool — most important knob, use ~60-70% of RAM
INNODB_BUFFER_POOL_INSTANCES=8 # 1 per core 
INNODB_READ_IO_THREADS=8 # 1 per core
INNODB_WRITE_IO_THREADS=8 # 1 per core
INNODB_PURGE_THREADS=4 # 1/2 per core
